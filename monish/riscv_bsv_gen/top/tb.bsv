...
// tb.bsv -- single Bluespec integration testbench for the RV32IM SoC.
// ----------------------------------------------------------------------------
// Verifies the whole processor in Bluesim:
//   * Two synchronous clocks: fast CPU (default) + slower Debug (divided).
//   * Loads a program (selected by TEST_* macro) into memory via the backdoor.
//   * Runs the core; each program ends in a self-loop (JAL x0,0) = "done".
//   * Uses the DEBUG interface (slow clock) to halt, read GPRs/memory, GPR/mem
//     write round-trips, and single-step -- exercising CDC + every abstract cmd.
//   * A scoreboard compares observed state against golden values -> PASS/FAIL.
//   * A watchdog aborts on hang.
//
// Build:  bsc -sim -g mkTb -u top/tb.bsv                 (default TEST=alu)
//         bsc -D TEST_MULDIV -sim -g mkTb -u top/tb.bsv  (another program)
//
// bsc 2025.01.1 compatibility notes:
//   * The program table is built with List::cons chains only -- no Vector
//     `vec(...)` constructor -- so it elaborates on any bsc version.
//   * The function is named getProgram: `program` is a reserved
//     (System)Verilog keyword and must not be used as a BSV identifier.
